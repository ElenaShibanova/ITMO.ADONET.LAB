...
namespace DBConnection
{
    public partial class Form1 : Form
    {
        ...
        {
            connectionToolStripMenuItem1.Enabled = (e.CurrentState == ConnectionState.Closed);
            asyncConnectionToolStripMenuItem.Enabled = (e.CurrentState == ConnectionState.Closed);
            unconnectionToolStripMenuItem.Enabled = (e.CurrentState == ConnectionState.Open);
        }
        ...
        private async void asyncConnectionToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (connection.State != ConnectionState.Open)
        ...
        private void numProdButton_Click(object sender, EventArgs e)
        {
            using (connection)
            {
                ...
                try
                {
                    ...
                    numProdLabel.Text = number.ToString();
                }
        ...
        private void numProdButton2_Click(object sender, EventArgs e)
        {
            try
            {
                int number = WorkWithDataBase.ExecuteScalarMetod(connectionString, "SELECT COUNT(*) FROM Products");
                numProdLabel2.Text = number.ToString();
            }
            catch (SqlException ex)
            {
                MessageBox.Show(ex.Message, "Ошибка!", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void listProdButton_Click(object sender, EventArgs e)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ListViewItem newItem = listProdListView.Items.Add(reader["ProductName"].ToString());
